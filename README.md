...
> [!IMPORTANT]
> 🚀 **[View the Project Presentation on Canva](https://canva.link/jg26ggj0ddbn2p4)**  
> 🎥 **[Watch the Project Video on YouTube](https://youtu.be/_g_MRXHrhHA)**
...
- 🔑 **API Keys**: You will need active accounts/keys for Firebase, Gemini AI, VirusTotal, and Numverify.

### 2️⃣ Environment Setup (.env)
Before running the app, you must set up the environment variables:
1. In the root folder, copy `.env.example` and rename the new file to `.env`.
2. Open the `.env` file and fill in all the required API keys (Firebase config, Gemini, VirusTotal, Numverify).

### 3️⃣ Install & Start Services
...
- ⚖️ `src/components/ModeratorDashboard.jsx`: Moderation queue, indicator auto-blacklisting, and community threat alert publisher
...
- ⚙️ `src/utils/rulesEngine.js`: Hybrid risk assessment scoring engine, keyword detectors, external API (Numverify/CCID) validators, and blacklist overrides
...
- **Frontend & Web App**: ⚛️ React 19, ⚡ Vite, 🎨 Lucide Icons
...
- **External Integrations**: 🛡️ VirusTotal API (URL checking), 📱 Numverify API (Phone line validation), 🚓 Mock CCID JSON Database (Simulated Police records)
- **Accessibility & Voice**: 🗣️ Web Speech API (Text-to-Speech Audio Synthesis), 🧓 Elderly Mode
- **State & Storage**: ☁️ Firebase Firestore (Real-time sync for Blacklists, Audit Logs, Reports, Alerts)
...
- 📸 **Multi-Format Scam Checker**: Pasted text, URL, and Phone number checks.
...
- 🛠️ **Admin Bulk Actions & Moderation**: Single-column structured admin layout with deduplicated queue management and bulk archiving.
...
- 🗺️ **Interactive Regional Map**: State-wide incident mapping across Malaysia (Kuala Lumpur, Selangor, Johor, Penang, Perak).

---

## 🌍 SDG Alignment
- 🕊️ **SDG 16 (Peace, Justice and Strong Institutions)**: Reductions in fraud victimization via transparent evidence checking.
- 🏗️ **SDG 9 (Industry, Innovation and Infrastructure)**: Secure local data integration.
